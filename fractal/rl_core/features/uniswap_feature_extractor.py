...
class UniswapFeatureExtractor:
    ...
    def _update_history(self, observations: Dict[str, float]):
        """Update price history with new observations."""
        # Extract values directly from dictionary
        self.price_history.append(observations["price"])
        self.high_price_history.append(observations["high_price"])
        self.low_price_history.append(observations["low_price"])
        self.close_price_history.append(observations["close_price"])
    ...
    def _calculate_price_ratios(
        self, high_price: float, low_price: float, open_price: float, close_price: float
    ...
    def _calculate_ewma_volatility(self, prices: List[float]) -> float:
        ...
        returns = np.diff(np.log(prices_array + 1e-10))  # Add small epsilon to avoid log(0)
    ...
    def _calculate_moving_averages(self, prices: List[float]) -> Tuple[float, float]:
        ...
        ma24 = float(np.mean(prices[-24:]))
        ma168 = float(np.mean(prices[-168:]) if len(prices) >= 168 else ma24)
    ...
    def _calculate_bop(
        self, prices: List[float], observations: Dict[str, float]
    ) -> float:
        """Calculate Balance of Power."""
        if len(prices) < 2:
            return 0.0
        high = observations["high_price"]
        low = observations["low_price"]
        close = observations["close_price"]
        open_price = observations["open_price"]
    ...
    def _calculate_true_range(
        ...
        if len(high_history) < 2 or len(low_history) < 2 or len(close_history) < 2:
            return 0.0
        # Convert to numpy arrays and ensure we have valid data
        high = np.array(high_history[-2:], dtype=np.float64)
        low = np.array(low_history[-2:], dtype=np.float64)
        close = np.array(close_history[-2:], dtype=np.float64)
    ...
    def _calculate_hilbert_transform(
        ...
        if len(prices) < period * 2:
            return 0.0, 0.0
        # Convert to numpy array and ensure we have valid data
        prices_array = np.array(prices[-period * 2:], dtype=np.float64)
        ...
        if len(peaks) > 0:
            dominant_cycle = float(peaks[0])
        else:
            dominant_cycle = float(period)
        ...
        return dominant_cycle, phase

    def forward(self, observations: Dict[str, float]) -> np.ndarray:
        """
        Forward pass of the feature extractor.
        Takes raw observations and returns processed features.

        Args:
            observations: Dictionary containing raw market state observations with float values

        Returns:
            np.ndarray: Array of processed features
        """
        # Update history with new observations
        self._update_history(observations)

        # Calculate price ratios
        high_to_open, low_to_open, close_to_open = self._calculate_price_ratios(
            observations["high_price"],
            observations["low_price"],
            observations["open_price"],
            observations["close_price"]
        )

        # Calculate all statistics first
        balance = observations["balance"]
        is_position = observations["is_position"]
        price = observations["price"]
        centralized_price = observations["centralized_price"]
        volume = observations["volume"]

        # Technical indicators
        dema = self._calculate_dema(self.price_history)
        dema_ratio = float(dema / price if price != 0 else 1.0)

        psar = self._calculate_parabolic_sar(self.high_price_history, self.low_price_history)
        psar_ratio = float(psar / price if price != 0 else 1.0)

        dx = float(self._calculate_dx(self.high_price_history, self.low_price_history, self.close_price_history))
        apo = float(self._calculate_apo(self.price_history))
        aroon = float(self._calculate_aroon_oscillator(self.high_price_history, self.low_price_history))
        bop = float(self._calculate_bop(self.price_history, observations))
        cci = float(self._calculate_cci(self.high_price_history, self.low_price_history, self.close_price_history))
        cmo = float(self._calculate_cmo(self.price_history))
        momentum = float(self._calculate_momentum(self.price_history))
        trix = float(self._calculate_trix(self.price_history))
        ult_osc = float(self._calculate_ultimate_oscillator(
            self.high_price_history,
            self.low_price_history,
            self.close_price_history
        ))
        ...
        k, d, slow_d = float(k), float(d), float(slow_d)

        # Hilbert Transform
        dominant_cycle, phase = self._calculate_hilbert_transform(self.price_history)
        dominant_cycle, phase = float(dominant_cycle), float(phase)

        # NATR
        natr = float(self._calculate_natr(self.high_price_history, self.low_price_history, self.close_price_history))

        # True Range
        true_range = float(self._calculate_true_range(
            self.high_price_history,
            self.low_price_history,
            self.close_price_history
        ))
        ...
        # EWMA Volatility
        ewma_vol = float(self._calculate_ewma_volatility(self.price_history))

        # Moving Averages
        ma24, ma168 = self._calculate_moving_averages(self.price_history)
        ma24, ma168 = float(ma24), float(ma168)

        # Create and return features array
        return np.array([
            balance,
            is_position,
            price,
