...
class CPO(PPO):
    ...
    def __init__(
        ...
        self.impermanent_loss_lambda = torch.tensor(
        ...
        self.constraint_optimizer = torch.optim.Adam(
            [self.lambda_, self.nu, self.impermanent_loss_lambda], lr=constraint_lr
        )
    ...
    def _compute_constraint_violation(self, rollout_data):
        ...
        impermanent_losses = observations[
            ..., -1
        ]  # Assuming IL is the last feature in observations
        il_quantile = torch.quantile(impermanent_losses, self.impermanent_loss_quantile)
        impermanent_loss_constraint = torch.relu(
            il_quantile - self.max_impermanent_loss
        )
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                value_constraint, impermanent_loss_constraint = (
                    self._compute_constraint_violation(rollout_data)
                )
        ...
        self.logger.record(
            "train/value_constraint_violation", np.mean(value_constraint_violations)
        )
        self.logger.record(
            "train/impermanent_loss_violation", np.mean(impermanent_loss_violations)
        )
