...
clean-runs:
	...
	# Sweep gitignored files (data CSVs, generated outputs, .ipynb scratch)
	# under examples/. ``git clean -X`` leaves whitelisted fixtures
	# (e.g. BTC_hyperliquid.csv, tau_strategy_result.csv) intact because
	# they're explicitly un-ignored in .gitignore.
	@if [ -d .git ]; then \
	    git clean -fdX examples/ 2>/dev/null || true; \
	fi
